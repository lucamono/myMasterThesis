...
\ref{fig:distrib}. In fact, for each grid the network will extract a specific point with a predicted confidence score between 0 and 1, the higher the score and the more the point will be the candidate for grasping an object (obviously, where the confidence will be low, it is assumed that there are no objects in that region of image pixels). This confidence distribution
