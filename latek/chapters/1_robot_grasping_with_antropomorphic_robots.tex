...
within the industry itself; pick and place tasks allow the automation of the production process within the assembly lines and the safety of workers. One more example
...
Industrial robotics has completely revolutionized the entire market not only in terms of production costs but also in terms of quality of trade products. The human work has literally 
been replaced by machines and is crucial to have robots in all industries, so it is clear that the reliability and precision of robots play a key role in the final quality of the work,
...
In many cases of Pick and place tasks the camera is mounted on the robotic arm itself, moreover the platform on which the arm is fixed can be mobile. A concrete example of pick and place tasks
with mobile platform is shown in Figure \ref{fig:youbot_pp} and regards the Youbot used by our \emph{S.P.Q.R.} team in occasion of the \emph{Robocup GermanOpen2018} competition which took place in Magdeburg.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/1_robot_grasping_with_antropomorphic_robots/pick_and_place}
    \caption{\textbf{Pick and place example.} The robot performs objects' grasping to release they in specific containers. Taken from https://www.roboticstomorrow.com/tag/pick-and-place} 
...
As far as kitting applications are concerned, the concept is having robots preparing the material in kits, in this way other robots can utilize the kitted parts at full capacity. The advantage of this
approach compared to manual work is related to improve productivity, flexibility, and part flow control. Many techniques, in part forced, are adopted in order to grasp objects such as \emph{Path Recording} in 
...
\begin{figure}[H]
    ...
    \caption{\textbf{Kitting task example.} A robot assembles Swiss knives in 3 containers. Taken from https://www.active8robots.com/} 
    \label{fig:pick_and_place}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.22]{figures/1_robot_grasping_with_antropomorphic_robots/path_recording}
    \caption{\textbf{Path Recording example.} Picture shows an operator dealing with the correct positioning of the robot joints. Taken from https://dof.robotiq.com/} 
    \label{fig:pick_and_place}
\end{figure}
...
robot is certainly a constraint not to be overlooked, the cycle time for picking and placing these parts needs 5 to 15 parts per minute.  
\begin{figure}[H]
    \centering
    ...
    stacked automotive parts from a large bin. Taken from https://www.robotics.org/} 
    \label{fig:rbp}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/1_robot_grasping_with_antropomorphic_robots/rbp_alg}
    \caption{\textbf{Execution of RBP localization algorithm with a 3D sensor vision.} In this picture, 3D area sensor maps the positions of multiple parts in a bin. Taken from
    https://www.robotics.org/content-detail.cfm/Industrial-Robotics-Industry-Insights/.} 
    \label{fig:rbp_alg}
\end{figure}
Smart gripping technology is the final challenge that robots meet to move random bin picking into the mainstream. The problem is that one gripper often doesn’t solve every
random resting state of a part in a bin and this is the main reason for which the random-bin-picking is far from a solved problem.

\section{Tools}\label{sec:tools}
Robots need to solve two different problems when grasping object, the first related to vision and the second to grasping, the sensors mainly used in this
...
Humans have literally dominated planet Earth thanks to the flexibility and ease to grasp and work with tools. Just looking at our hands we find the answer, especially looking the
thumb, the only finger that is anatomically 90 degrees rotated on the coronal axis respect to other fingers (the Figure  \ref{fig:monkey} shows how difficult is for animals, in this case 
a monkey, grasping a common object with their hands). Equipped only with two phalanges, i.e., proximal phalanx and distal phalanx, Thumb is able to oppose orientation with the other
four fingers.  
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.3]{figures/1_robot_grasping_with_antropomorphic_robots/monkey}
    \caption{\textbf{A monkey grabbing a stick.} The opposable thumb differentiates man from the rest of the animal world, making us the most evolved species in the world able to grasp and manipulate objects.} 
    \label{fig:monkey}
\end{figure}
With the advent of technology, the anatomical study of the hand has allowed us to create machines that can grab objects in the most comfortable way, a robot with various different end-effectors installed 
imply different industri\	al uses related to the type of object treated. The majority of grippers are variations of three fundamental designs as parallel, three-finger and angled grippers. In this section
...
\begin{figure}[H]
    ...
    \caption{\textbf{Schunk WSG.} A servo-electric 2 finger parallel gripper with sensitive gripping force control and long stroke. Taken from http://www.directindustry.com/prod/schunk/product-12463-902655.html.} 
    \label{fig:par_grip}
\end{figure}
...
This tool uses three fingers. It is clear how its use is strictly limited to few categories of graspable objects, in particular the Figure \ref{fig:3_fing_grip} shows 
...
\begin{figure}[H]
    ...
    \caption{\textbf{Robotiq 3FAG.} The 3-Finger Adaptive Gripper is ideal for advanced manufacturing and robotic research. It adapts to the object’s shape for a solid grip. Taken from
    https://robotiq.com/support/3-finger-adaptive-robot-gripper/downloads-instruction-manual.} 
...
Micro and Nano grippers are needed for manipulation of microscopic samples such as in biology or micro assembly. Most micro manipulators rely on the principle of micro 
...
\end{figure}
Thanks to CMOS, ToF sensors allow to obtain a full depth in real time, excellent for mobile robots especially during the SLAM phase. Obviously and unfortunately against, the quality is generally low and
accuracy is not always garantle especially with reflective materials such as metals or black surfaces. 
\begin{figure}[H]
    \centering
...
As far as Structured Light sensors are concerned, this kind of sensor is based on light triangulation. These devices encompass many different system, due their geometry and
...
and so on. Hence, they are essentially depth sensors incorporating a laser projector inside that can project patterns of light. In this way, knowing the static distance between
