...
independently, and the choice of the best grasping pose. Given a cad models, typical approaches is based on estimating the pose of an object by matching with these models. Hence, the pose obtained
is setted according to the position and orientation in which the objects are located. We are faced with a clear problem of computer vision, a fundamental element to allow the robot to identify the region in the 
space where the gripper will perform the grasping. Learning systems such as deep learning, especially CNNs, have reached the state of the art in robot grasping. Using the resources made available in the
\emph{RoCoCo} laboratory, \emph{Sapienza}, University of Rome, including a \emph{Microsoft Kinect2} camera rgb-depth and a \emph{ROBOX} 6 d.o.f. antropomorphic robot manipulator, this thesis work aims to solve the
grasping problem of unknown objects by employing a supervised, data driven learning approach. Given an RGB image  $\mathcal{I}$ and the corresponding depth map $\mathcal{D}$, we aim to estimate the best grasping 
position. Hence this project introduces a new dataset consisting of 4000 acquisitions in which each scenario is related to a robot attempting to grasp an object. This dataset provides
...
and trained by using TensorFlow deep learning framework. All this huge work aims at converging the predicted outputs by the network on the same wavelength of which the same title of this thesis work makes 
