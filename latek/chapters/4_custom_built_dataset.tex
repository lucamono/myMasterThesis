...
In this section all the parts related to the dataset and its acquisition software will be exposed. Dataset's acquisition includes a large part of this thesis' work. The software
developed must take into account many factors such as security, reliability, ductility and so on; these components are strictly necessary as the dataset must be robust to any false 
positives that could compromise the overall quality.
\begin{figure}[H]
    \centering
...
interest in the matter, it is good remember the three Asimov's laws of robotics, i.e., the set of rules devised by the science fiction author Isaac Asimov, \cite{asimov}:
\begin{itemize}
 \item \textbf{I)} \emph{A robot may not injure a human being or, through inaction, allow a human being to come to harm.}
 \item \textbf{II)} \emph{A robot must obey the orders given it by human beings except where such orders would conflict with the First Law.}
...
It was decided to create a completely autonomous acquisition software able to operate inside the Rococo laboratory. (DIAG, Sapienza) for hours and hours without the help of human
...
In this subsection the setup of the acquisition environment will be explained in detail. The setup consists in a custom built cell with an RGB-D sensor statically mounted on a top
cell rod and an anthropomorphic manipulator inside. Hence, we will see the calibration and the relationship between the camera and the robot. To conclude this subsection, the working
...
The robot used for this project of thesis is a 6 d.o.f. antropomorphic manipulator, in particular, the robot cell is shows in figure \ref{fig:robot_cell}. This manipulator and its own
control system have been provided by Robox, by the way, the controller allows us to interface the robot with our acquisition software via serial communication so we were able to send 
...
\begin{figure}[H]
    ...
    \label{fig:kinect2}
\end{figure}
Like its predecessor, alias the \emph{Microsoft Kinect v1}, this device is based on the issuance of a divergent dense pattern as a pseudo-random dot matrix pattern. Table \ref{tab:kin_2} shows the technical features
of Kinect v2.
\begin{table}[H]
 \centering
...
\begin{figure}[H]
    ...
    \label{fig:sens_loc}
\end{figure}
Once the external parameters of the Kinect2 have been obtained, we can proceed to the Hand-eye calibration. In this kind of calibration (useful in many cases as for example, grasping
...
the camera to the hand coordinate system need to be estimated simultaneously, in our case, by applying the transformation of the target pose in Kinect2 image coordinates
...
conducted by placing each object at random positions and orientations within a graspable area of the board upon which the robot was mounted.
\begin{figure}[H]
    \centering
...
\end{figure}
\subsection{Acquisition protocol}\label{subsec:acquisition_protocol}
To execute the dataset's acquisition, a series of attempting to grasp objects have been performed. First of all, a random point was chosen within a region proposal. This possible candidate point must be analized
with a check security on the depth-map to avoid collision with the environment and objects. In the final step, after the depth convalidation, the kinematic controls are sent for this target point allowing the robot
to perform the attempt of grasping. In the end the dataset is updated and the grasp is be counted as successful if an object is be lifted off the board to a height of 30cm, 

\subsection{Region proposal}\label{subsec:region_proposal}
During the acquisition of the dataset it is necessary to know how to manage the ratio of objects grasped or not expecially for drive the CNN's training into the correct behaviour. Considering the limitations of
...
\begin{figure}[H]
    ...
    \label{fig:reg_prop}
\end{figure}
This process is realized using the YOLO neural network, described in detail in section \ref{sec:yolo}, which is agnostic with respect to the objects used for
...
are not efficient in terms of depth measurements with particular materials such as glass, transparent plastic, reflective metals or  black color objects. the \emph{src} folder of the 
\emph{LM Grasping software} contains the \emph{imageProcessing.cpp} file which in turn contains the \emph{checkDepthIsSafe} method. This method is basically based to performs a check
...
\subsection{Dataset structure}\label{subsec:dataset_structure}
...
\end{figure}
 ...
 figure \ref{fig:regDepth}. Also in this case, the 4 channels of the png format preserve the numerical information of the depthMap acquired.
    \begin{figure}[H]
    \centering
...
\end{figure}
 \item \textbf{data}: A txt file containing all the grasping information such as the target pose in the various frames, the depth value, the pitch and roll orientations target for the gripper and the binary information
