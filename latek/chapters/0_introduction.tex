...
From birth grabbing objects for us humans has been an extremely simple task but imagine the impact that could cause a future where robots would be able to grab objects with the
same ease, we would definitely talk about a new industrial revolution, with changes in all areas of major interest such as freight transport, rescue operations, medical and welfare. 
This reflection lies in the fact that robot grasping is not a solved problem. Recent studies, with aim the computation of the best pose of grasping from images that contains 
objects, have encountered many accuracy problems due to the difficulty of making the machine learn the right behaviour in different scenarios, in which for example, different 
objects can find themselves in uncomfortable positions to be grasped. In addition, factors such as incorrect calibration between the camera and robot, joint encoders, radial 
distortion and other causes, heavily affect the grasping task. Lately deep learning has taken root in robot grasping and has become a reference point for many studies and research.
Taking inspiration from existing CNNs and related works we worked intensively on the concept of estimation of the best pose for grasping objects. We have optimized fundamental
aspects such as acquisition setups, custom CNN builded and a new loss function optimized for this project, all indispensable resources that have made this thesis project possible.

\section{Problem Statement and Motivations }\label{sec:motivations}
We have always been attracted of science fiction movies in which robots were undisputed protagonists of the scene, able to serve, interact and make choices independently. 
Unfortunately, the reality is very different from what we see in the films, the robots are still not able to understand or to perform unexpected actions from logic until to reach
the concept of rebellion. Robot grasping has always been a difficult task to accomplish. In addition to the problem of grasping, and therefore of all the forces to be applied to the gripper and 
the robot joints, vision plays a fundamental role in the localization of the object and consequently the best grasping point available. Grasping in robotics is then split as a problem related to two 
essential components belonging to a robot:
\begin{itemize}
 \item \textbf{The eye of the robot}: we are talking about the part related to the computer vision, indispensable for the understanding of the surrounding environment and the
 localization of objects
 \item \textbf{The arm with the hand of the robot}: we are talking about the robot's parts related to the mechanical, structure, joints, controller, end effector grippers and
 so on; all the components related to the physical object grasping 
\end{itemize}
\begin{figure}[H]
    \centering
...
Currently, in the calendar year 2018 A.D., neural networks are making great strides to artificial intelligence, emulating the functioning of the human brain through artificial neurons. These networks are becoming 
in fact the state of the art for object detection and robotic grasping due to the computing power of computers equivalent to about 3 million of human brain neurons. When this capability reaches the number of 
neurons that the human brain possesses, about 50 billion, there will probably be to reinvent the concept of ethics and religion; for the moment in this thesis we will focus on the creation of a neural network
capable of predicting, given as input a color-depth image, the best point of grasping for unknown objects.

\section{Related Works }\label{sec:related_works}
In this section all the works related to the robot grasping will be exposed. In particular, we will start by illustrating two sections: the first will include many contributions over the years, despite the limited 
...
In \cite{research_grasp}, back in 2000 A.D., the authors focused on issues that are central to the mechanics of grasping and the finger-object contact interactions. This research has established the theoretical framework for grasp
analysis, simulation and synthesis by opening up interesting scenarios for future works. In \cite{hybrid_approach}, the autors proposed a control scheme as the fusion of the sensor 
...
\begin{figure}[H]
    ...
    \caption{\textbf{The ARMAR-III grasping.} ARMAR-III is grasping a mashed potatoes box, captured by the robot’s camera system.} 
    \label{fig:armar}
\end{figure}
...
robot arm control training, deep learning features for robot localisation, and so on. Despite the invaluable benefits by using these methods in robotics, robot grasping is very far from a problem solved. Most of the work and
...
\begin{figure}[H]
    ...
    \caption{\textbf{The Cornell Dataset.} On Cornell Grasping Dataset, each object has multiple labelled grasps.} 
    \label{fig:cornell}
\end{figure}
...
\begin{figure}[H]
    ...
    numeric quality measures.} 
    \label{fig:graspit}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/0_introduction/dart}
    \caption{\textbf{DART simulator.} DART simulator includes physical forces and dynamic modelling during the grasping.} 
    \label{fig:dart}
\end{figure}
...
This thesis is the result of work born with time, passion for computer vision and the desire to extend on a real robot the ability to perceive the world around as well as to grasp objects independently. I had the 
opportunity to get passionate about grasping from my first experience in Robocup, as a member of the team @Work that targets the use of robots in work-related scenarios, as depicted in figure \ref{fig:robocup}. 
From here, after this personal and fantastic experience, learn how to grasp objects, obviously in scenarios of robotics and artificial intelligence, has become in fact a priority and a goal for my studies. In
particular, this thesis focuses on two sequential steps: the first regards the acquisition of a new grasping dataset; the second, the creation of a new Convolutional Neural Network (CNN) suitable for the features
of the dataset, with the final goal to learn the $(x,y)$ point of grasping coordinates by performing regression on the entire image. The dataset was acquired by real attempts to grab objects on a scene, taking 
...
work it has been chosen to use a vacuum end effector as gripper and this means that only a singular point of grasping is available due to the shape of the gripper. Hence, the estimation of the x-y coordinates needs 
...
\begin{figure}[H]
    ...
    \caption{\textbf{Spqr@Work team.} Me with my team during the Robocup2018 GermanOpen competition in Magdeburg.} 
    \label{fig:robocup}
\end{figure}