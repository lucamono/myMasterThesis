...
Robot grasping is closely related to a problem of vision perception because the robot must identify the region in the space in which the gripper will have to grasp the object. The success of a common grasping task
depends from the accuracy of the calibration between the camera sensor and the robotic arm, and also from all the forces applied to the gripper and the robot joints.
Grasping task is then split as a problem related to two essential components:
\begin{itemize}
 ...
 \item \textbf{The arm with the hand of the robot}: we are talking about robot's parts related to mechanical, structure, joints, controller, end effector grippers and so on; all the components related to physical
 object grasping. 
\end{itemize}
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \caption{\textbf{Matching CAD model example.} An example of matching between a CAD models and objects.} 
    \label{fig:d2co}
\end{figure}
